...
WITH
cte1 AS(
SELECT COUNT(DISTINCT sender_id, send_to_id) AS cnt1 FROM FriendRequest),

cte2 AS(
SELECT COUNT(DISTINCT requester_id, accepter_id) AS cnt2 FROM RequestAccepted)


SELECT IF(cnt2=0, 0.00, ROUND(cnt2/cnt1,2)) AS accept_rate
